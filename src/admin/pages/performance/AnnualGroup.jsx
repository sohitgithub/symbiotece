...
// ✅ CSS Import (Adjust path if needed)
...
export default function AnnualGroup() {
  ...
  const initialFormState = {
    financialYear: "",
    documentType: "Consolidated Report", // Default changed for Group
    status: "Draft",
    pdfFile: null,
  };
  ...
  // Options specific to Group
  const docTypeOptions = ["Consolidated Report", "Group Financials", "Annual Return"];
  ...
  const fetchResults = async () => {
    setIsLoading(true);
    try {
      // ✅ API Endpoint specific to Annual Group
      let url = `${baseUrl}/api/annual-group`; 
  ...
  const handleSubmit = async (e, isEdit = false) => {
    ...
    const url = isEdit ? `${baseUrl}/api/annual-group/${selectedItem.id}` : `${baseUrl}/api/annual-group`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsLoading(true);
    try {
        const res = await fetch(`${baseUrl}/api/annual-group/${selectedItem.id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
        });
  ...
  return (
    <div className="section-page-container">
      <div className="section-header">
        <div>
          <h1 className="page-title">Performance <span className="subtitle">— Annual Group</span></h1>
          <p className="page-description">Manage Consolidated Group Reports</p>
        </div>
      ...
      {showCreateModal && (<div className="modal-overlay"><div className="modal-card"><h2 className="modal-title">New Group Report</h2>{renderModalForm(false)}</div></div>)}
