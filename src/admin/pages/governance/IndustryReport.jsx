...
export default function IndustryReport() {
  ...
  const fetchResults = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${baseUrl}/api/industry-reports`, { 
  ...
  const handleSubmit = async (e, isEdit = false) => {
    ...
    const url = isEdit ? `${baseUrl}/api/industry-reports/${selectedItem.id}` : `${baseUrl}/api/industry-reports`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsLoading(true);
    try {
        const res = await fetch(`${baseUrl}/api/industry-reports/${selectedItem.id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const renderModalForm = (isEdit) => (
    <form className="modal-form" onSubmit={(e) => handleSubmit(e, isEdit)}>
      <div className="form-group">
        ...
        <input type="text" name="title" value={formData.title} onChange={handleInputChange} required placeholder="e.g. Pharma Industry Report 2024" />
      </div>
       <div className="form-group">
  ...
  return (
    <div className="section-page-container">
      <div className="section-header">
        <div>
          <h1 className="page-title">Governance <span className="subtitle">— Industry Reports</span></h1>
          <p className="page-description">Manage Industry Research Reports</p>
        </div>
      ...
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
            {results.length === 0 ? (
                <tr><td colSpan="3" style={{textAlign:"center", padding:"20px"}}>No reports found</td></tr>
            ) : (
                results.map((item) => (
                <tr key={item.id}>
      ...
      {showCreateModal && (<div className="modal-overlay"><div className="modal-card"><h2 className="modal-title">Add Report</h2>{renderModalForm(false)}</div></div>)}
