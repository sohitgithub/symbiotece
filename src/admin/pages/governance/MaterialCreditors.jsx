...
export default function MaterialCreditors() {
  ...
  const fetchResults = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${baseUrl}/api/material-creditors`, { 
  ...
  const handleSubmit = async (e, isEdit = false) => {
    ...
    const url = isEdit ? `${baseUrl}/api/material-creditors/${selectedItem.id}` : `${baseUrl}/api/material-creditors`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsLoading(true);
    try {
        const res = await fetch(`${baseUrl}/api/material-creditors/${selectedItem.id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const renderModalForm = (isEdit) => (
    <form className="modal-form" onSubmit={(e) => handleSubmit(e, isEdit)}>
      <div className="form-group">
        ...
        <input type="text" name="title" value={formData.title} onChange={handleInputChange} required placeholder="e.g. List of Material Creditors" />
      </div>
       <div className="form-group">
  ...
  return (
    <div className="section-page-container">
      <div className="section-header">
        <div>
          <h1 className="page-title">Governance <span className="subtitle">— Material Creditors</span></h1>
          <p className="page-description">Manage Material Creditors Documents</p>
        </div>
