...
export default function ShareholdingPattern() {
  ...
  const fetchResults = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${baseUrl}/api/shareholding-patterns`, { 
  ...
  const handleSubmit = async (e, isEdit = false) => {
    ...
    const url = isEdit ? `${baseUrl}/api/shareholding-patterns/${selectedItem.id}` : `${baseUrl}/api/shareholding-patterns`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsLoading(true);
    try {
        const res = await fetch(`${baseUrl}/api/shareholding-patterns/${selectedItem.id}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const renderModalForm = (isEdit) => (
    <form className="modal-form" onSubmit={(e) => handleSubmit(e, isEdit)}>
      <div className="form-group">
        <label>Title</label>
        <input type="text" name="title" value={formData.title} onChange={handleInputChange} required placeholder="e.g. Shareholding Pattern Q1 2024" />
      </div>
       <div className="form-group">
  ...
  return (
    <div className="section-page-container">
      <div className="section-header">
        <div>
          <h1 className="page-title">Governance <span className="subtitle">— Shareholding Pattern</span></h1>
          <p className="page-description">Manage Shareholding Documents</p>
        </div>
      ...
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
            {results.length === 0 ? (
                <tr><td colSpan="3" style={{textAlign:"center", padding:"20px"}}>No documents found</td></tr>
            ) : (
                results.map((item) => (
                <tr key={item.id}>
      ...
      {showCreateModal && (<div className="modal-overlay"><div className="modal-card"><h2 className="modal-title">Add Pattern</h2>{renderModalForm(false)}</div></div>)}
      {showEditModal && (<div className="modal-overlay"><div className="modal-card"><h2 className="modal-title">Edit Pattern</h2>{renderModalForm(true)}</div></div>)}
      
       {showDeleteModal && selectedItem && (
        <div className="modal-overlay">
          <div className="modal-card delete-modal-card">
             <h2 className="modal-title">Delete Pattern</h2>
